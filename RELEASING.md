...
This guide describes how to release a new version of timelens to PyPI.

## Prerequisites

1. **PyPI Trusted Publishing configured**
   - Go to https://pypi.org/manage/project/timelens/settings/publishing/
   - Add a new publisher:
     - Owner: `thec0dewriter`
     - Repository: `timelens`
...
2. **TestPyPI Trusted Publishing configured** (optional, for testing)
   - Go to https://test.pypi.org/manage/project/timelens/settings/publishing/
...
Edit `src/timelens/_version.py` and update the version:
...
```bash
git add src/timelens/_version.py
...
Verify at https://test.pypi.org/project/timelens/

Test installation:
```bash
pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple/ timelens
```
...
- Check https://pypi.org/project/timelens/
- Check GitHub Releases page
- Test installation: `pip install timelens==0.2.0`
...
The git tag (e.g., `v0.2.0`) must match the version in `src/timelens/_version.py` (e.g., `0.2.0`).
...
# Fix version in src/timelens/_version.py, commit, then re-tag
git tag v0.2.0
git push origin v0.2.0
```
...
Check that `src/timelens/` is included in the wheel.
